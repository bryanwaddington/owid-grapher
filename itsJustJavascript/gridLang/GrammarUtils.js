...
exports.tsvToMatrix = exports.didYouMean = exports.isEmpty = exports.isBlankLine = void 0;
const isBlankLine = (line) => line === undefined ? true : line.join("") === "";
...
const didYouMean = (str = "", options = [], caseSensitive = false, threshold = 0.4, thresholdAbsolute = 20) => {
    if (!caseSensitive)
        str = str.toLowerCase();
    // Calculate the initial value (the threshold) if present.
    let maximumEditDistanceToBeBestMatch = Math.min(threshold * str.length, thresholdAbsolute);
    ...
    for (let optionIndex = 0; optionIndex < len; optionIndex++) {
        ...
        if (!candidate)
            continue;
        const editDistance = getEditDistance(str, caseSensitive ? candidate : candidate.toLowerCase(), maximumEditDistanceToBeBestMatch);
...
const getEditDistance = (stringA, stringB, maxInt) => {
...
const delimitedToMatrix = (delimited, rowDelimiter = "\n", columnDelimiter = "\t") => delimited.split(rowDelimiter).map((line) => line.split(columnDelimiter));
exports.tsvToMatrix = delimitedToMatrix;
//# sourceMappingURL=GrammarUtils.js.map