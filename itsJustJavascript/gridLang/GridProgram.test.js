...
const GridProgram_1 = require("./GridProgram");
describe(GridProgram_1.GridProgram, () => {
    it("can create a program", () => {
        const program = new GridProgram_1.GridProgram("test", "");
        expect(program.lines.length).toEqual(1);
    });
    it("can set values", () => {
        const program = new GridProgram_1.GridProgram("test", "title\thello world");
        expect(program.getLineValue("title")).toEqual("hello world");
        expect(program.setLineValue("title", "good morning").getLineValue("title")).toEqual("good morning");
    ...
    it("can find rows", () => {
        expect(new GridProgram_1.GridProgram("test", `table\ntableName\ntable\tall\n`).getRowNumbersStartingWith("table")).toEqual([0, 2]);
    });
    describe("blocks", () => {
        const program = new GridProgram_1.GridProgram("test", `table
\tslug
\tcountry`);
        it("can get blocks", () => {
...
country\tCountry`;
        ...
        it("can delete blocks", () => {
            ...
            const program2 = new GridProgram_1.GridProgram("test", `table
\tslug
\tcountry
columns
\tone`);
...
\tslug`);
        it("can loop", () => {
            ...
            expect(Array.from(program.valuesFrom({ row: 100, column: 10 }))).toEqual(expected);
            expect(Array.from(new GridProgram_1.GridProgram("est", "").valuesFrom({ row: 1, column: 1 }))).toEqual([""]);
            expect(Array.from(new GridProgram_1.GridProgram("est", "a\nb").valuesFrom({
                row: 1,
                column: 0,
            }))).toEqual(["b", "a"]);
            expect(Array.from(program.valuesFrom({ row: 1, column: 2 }))).toEqual(["columns", "", "slug", "table", "", "slug"]);
        });
        it("can grep", () => {
...
//# sourceMappingURL=GridProgram.test.js.map