...
describe("creating tables", () => {
    it("can create tables from csv", () => {
        const table = new CoreTable_1.CoreTable(sampleCsv);
    ...
    it("can create tables from csv with columns definitions also as csv", () => {
        const table = new CoreTable_1.CoreTable(sampleCsv, `slug,name
country,Region
population,Population in 2020`);
    ...
    describe("transforms", () => {
        it("can create columns from transforms", () => {
            const table = new CoreTable_1.CoreTable(sampleCsv, `slug,name,transform
country,Region,
population,Population in 2020,
popTimes10,Pop times 10,multiplyBy population 10`);
        ...
        describe("runs transforms just once", () => {
            const table = new CoreTable_1.CoreTable(`country,population
iceland,1
iceland,2
iceland,3
france,50
france,60
france,75`, `slug,name,transform
country,Region,
population,Population in 2020,
popChange,Pop change,percentChange time country population 2`);
            const expected = [
                ErrorValues_1.ErrorValueTypes.NoValueToCompareAgainst,
                ErrorValues_1.ErrorValueTypes.NoValueToCompareAgainst,
                200,
                ErrorValues_1.ErrorValueTypes.NoValueToCompareAgainst,
                ErrorValues_1.ErrorValueTypes.NoValueToCompareAgainst,
            ...
            it("runs transforms correctly", () => {
                expect(table.get("popChange").valuesIncludingErrorValues).toEqual(expected);
            });
            it("runs transforms once", () => {
                expect(table
                    ...
                    .get("popChange").valuesIncludingErrorValues).toEqual(expected.slice(1));
            });
        });
    });
    it("can create an empty table", () => {
        expect(new CoreTable_1.CoreTable().transformCategory).toEqual(CoreTableConstants_1.TransformType.LoadFromRowStore);
    });
    it("will only look at properties of the first object literal when autodetecting columns from rows", () => {
        const table = new CoreTable_1.CoreTable([{ name: "test" }, { score: 123 }]);
        expect(table.columnNames).toEqual(["name"]);
    });
    it("can combine tables", () => {
        const table = new CoreTable_1.CoreTable(sampleCsv).concat([
            new CoreTable_1.CoreTable(sampleCsv),
        ]);
        expect(table.numRows).toEqual(8);
    });
    it("can drop empty rows", () => {
        const table = new CoreTable_1.CoreTable(`country,gdp\nusa,123\n,\n`);
        expect(table.dropEmptyRows().numRows).toEqual(1);
    });
    it("can create a table from columns", () => {
        const table = new CoreTable_1.CoreTable({
            scores: [0, 1, 2],
    ...
    it("can handle when a blank column type is provided", () => {
        expect(new CoreTable_1.CoreTable("", [{ slug: "gdp", type: "" }]).numRows).toEqual(0);
    });
    it("always parses all values in all rows to Javascript primitives when the table is initially loaded", () => {
        ...
        const table = new CoreTable_1.CoreTable(rows);
...
123,can`;
    ...
    describe("loading from matrix", () => {
        ...
        const table = new CoreTable_1.CoreTable(matrix);
        ...
        const tableTrim = new CoreTable_1.CoreTable([
            ["country", null],
            ["usa", undefined],
        ]);
    ...
    it("handles ErrorValues when serializing to a matrix", () => {
        const table = new CoreTable_1.CoreTable([{ country: "usa", gdp: undefined }]);
        expect(table.toMatrix()[1][1]).toEqual(undefined);
    });
    it("can transpose a table", () => {
        let table = new CoreTable_1.CoreTable([
    ...
    it("can create a table with columns but no rows", () => {
        expect(new CoreTable_1.CoreTable({}, [{ slug: "entityId" }]).get("entityId").values).toEqual([]);
    });
    it("can create a table with an empty column", () => {
        const table = new CoreTable_1.CoreTable([{ color: "blue" }], [{ slug: "name", type: CoreColumnDef_1.ColumnTypeNames.String }]);
...
describe("set methods", () => {
    it("can find the intersection between 2 tables", () => {
        const table = new CoreTable_1.CoreTable(sampleCsv);
        expect(table.intersection([new CoreTable_1.CoreTable(sampleCsv)]).numRows).toEqual(4);
        expect(table.intersection([new CoreTable_1.CoreTable()]).numRows).toEqual(0);
        expect(table.intersection([new CoreTable_1.CoreTable(sampleCsv), new CoreTable_1.CoreTable()])
            .numRows).toEqual(0);
        expect(table.intersection([new CoreTable_1.CoreTable(sampleCsv + "\n" + sampleCsv)])
            .numRows).toEqual(4);
        expect(table.intersection([
            new CoreTable_1.CoreTable(sampleCsv.replace("\ncanada,20", "")),
        ]).numRows).toEqual(3);
    });
    it("can perform a union", () => {
        const table = new CoreTable_1.CoreTable(sampleCsv);
        expect(table.union([new CoreTable_1.CoreTable(sampleCsv)]).numRows).toEqual(4);
        expect(table.union([
            new CoreTable_1.CoreTable([{ country: "Mexico", population: 20 }]),
        ]).numRows).toEqual(5);
    });
    it("can perform a diff", () => {
        const table = new CoreTable_1.CoreTable(sampleCsv);
        expect(table.difference([new CoreTable_1.CoreTable(sampleCsv)]).numRows).toEqual(0);
        const tb = new CoreTable_1.CoreTable([{ country: "Mexico", population: 20 }]);
    ...
    it("does not drop any rows if there are no duplicates", () => {
        expect(new CoreTable_1.CoreTable(sampleCsv).dropDuplicateRows().numRows).toEqual(4);
    });
});
...
usa,2002
uk,2001`;
    const table = new CoreTable_1.CoreTable(csv);
    expect(table.numRows).toEqual(3);
...
describe("adding rows", () => {
    describe("adding rows is immutable", () => {
        const table = new CoreTable_1.CoreTable(sampleCsv);
        expect(table.numRows).toEqual(4);
        let expandedTable = table.appendRows([{ country: "Japan", population: 123 }], `Added 1 row`);
        expect(expandedTable.numRows).toBe(5);
        expect(table.numRows).toEqual(4);
        it("can append rows", () => {
            expandedTable = expandedTable
                .renameColumns({ population: "pop" })
                .appendRows([{ country: "USA", pop: 321 }], "Added a row after column renaming");
...
describe("column operations", () => {
    ...
    it("can rename a column", () => {
        let table = new CoreTable_1.CoreTable([{ pop: 123, year: 2000 }]);
    ...
    it("does not mutate input rows ever", () => {
        ...
        expect(rows[0].countryNameLength).toEqual(undefined);
    });
    it("can drop columns", () => {
        const rows = [
    ...
    it("can transform columns", () => {
        ...
        expect(table.updateDefs((def) => {
            return Object.assign(Object.assign({}, def), { name: def.slug.toUpperCase() });
        }).columnNames).toEqual(["COUNTRY", "YEAR"]);
    });
    it("can sort columns", () => {
        const rows = [
            { country: "USA", year: 1999 },
...
describe("searching", () => {
    ...
    it("can do grep like searching to find rows", () => {
        ...
        expect(table.grep("Germany").grep("2001").opposite.rows[0].year).toEqual(2000);
    ...
    it("can get the domain across all columns", () => {
        const table = new CoreTable_1.CoreTable(`gdp,perCapita
0,123.1
12,300
20,40`, [
            { slug: "gdp", type: CoreColumnDef_1.ColumnTypeNames.Numeric },
            { slug: "perCapita", type: CoreColumnDef_1.ColumnTypeNames.Numeric },
        ]);
...
hi,1,,2001`;
        ...
        const entityNameMap = annotationsColumn.getUniqueValuesGroupedBy("entityName");
...
describe("filtering", () => {
    describe("row filter", () => {
        const rootTable = new CoreTable_1.CoreTable(sampleCsv);
        const filteredTable = rootTable.rowFilter((row) => parseInt(row.population) > 40, "Pop filter");
        it("can filter", () => {
        ...
        it("can chain filters", () => {
            const filteredTwiceTable = filteredTable.rowFilter((row) => row.country.startsWith("u"), "Letter filter");
        ...
        it("can filter all", () => {
            const table = new CoreTable_1.CoreTable(`country,pop
    usa,123
    can,333`);
    ...
    describe("column filter", () => {
        const rootTable = new CoreTable_1.CoreTable(sampleCsv);
        const filteredTable = rootTable.columnFilter("population", (v) => parseInt(v) > 40, "Pop filter");
        it("can filter", () => {
            expect(rootTable.get("country").values[3]).toEqual("canada");
            const parsedValues = filteredTable.get("country").values;
            expect(parsedValues[0]).toEqual("france");
        ...
        it("can chain filters", () => {
            const filteredTwiceTable = filteredTable.columnFilter("country", (v) => v.startsWith("u"), "Letter filter");
            const parsedValues = filteredTwiceTable.get("country").values;
            expect(parsedValues[0]).toEqual("usa");
            expect(parsedValues[1]).toEqual(undefined);
        });
        it("can filter error values", () => {
            const table = new CoreTable_1.CoreTable([
                ...
                { slug: "country", type: CoreColumnDef_1.ColumnTypeNames.String },
                { slug: "value", type: CoreColumnDef_1.ColumnTypeNames.Numeric },
            ]);
            expect(table.columnFilter("value", (v) => ErrorValues_1.isNotErrorValue(v), "filter out error values").numRows).toEqual(1);
        });
        it("can filter all", () => {
            const table = new CoreTable_1.CoreTable(`country,pop
            usa,123
            can,333`);
            const allFiltered = table.columnFilter("pop", () => false, "filter all");
            expect(allFiltered.get("pop").values).toEqual([]);
        });
    });
});
describe("debug tools", () => {
    const table = new CoreTable_1.CoreTable(sampleCsv).dropColumns(["population"]);
...
describe("printing", () => {
    it("uses slugs for headers in toDelimited", () => {
        const table = new CoreTable_1.CoreTable(`country,Population in 2020
iceland,1`);
...
Canada,2020-01-23,`);
    ...
    it("can format a value", () => {
        const table = new CoreTable_1.CoreTable(`growthRate
123`, [
            {
                slug: "growthRate",
                display: { unit: "%" },
...
123,123.1`);
    ...
    it("can get all defined values for a column", () => {
        const table = new CoreTable_1.CoreTable([
            ...
            { pop: 123, year: 2000 },
        ], [{ type: CoreColumnDef_1.ColumnTypeNames.Numeric, slug: "pop" }]);
...
describe("joins", () => {
    const leftTable = new CoreTable_1.CoreTable({
        country: ["usa", "can", "fra"],
        time: [2000, 2001, 2002],
        color: ["red", "green", "red"],
    });
    const rightTable = new CoreTable_1.CoreTable({
    ...
    it("can create indices", () => {
        ...
        expect((_a = index2.get("red usa")) === null || _a === void 0 ? void 0 : _a.length).toEqual(1);
    });
    describe("outer joins", () => {
        it("can left join on all intersecting columns", () => {
            expect(leftTable.leftJoin(rightTable).toTypedMatrix()).toEqual([
                ...
                ["usa", 2000, "red", 55],
                ["can", 2001, "green", 66],
                ["fra", 2002, "red", ErrorValues_1.ErrorValueTypes.NoMatchingValueAfterJoin],
            ]);
        });
        it("can left join on one column", () => {
            expect(leftTable.leftJoin(rightTable, ["time"]).toTypedMatrix()).toEqual([
                ["country", "time", "color", "population"],
                ["usa", 2000, "red", 55],
                ["can", 2001, "green", 66],
                ["fra", 2002, "red", 77],
        ...
        it("can do a right join", () => {
            expect(leftTable.rightJoin(rightTable).toTypedMatrix()).toEqual([
                ...
                ["usa", 2000, 55, "red"],
                ["can", 2001, 66, "green"],
                ["turk", 2002, 77, ErrorValues_1.ErrorValueTypes.NoMatchingValueAfterJoin],
            ]);
        });
    });
    ...
    describe("full join", () => {
        it("can do a full join", () => {
            expect(leftTable.fullJoin(rightTable).toTypedMatrix()).toEqual([
                ...
                ["can", 2001, "green", 66],
                ["fra", 2002, "red", ErrorValues_1.ErrorValueTypes.NoMatchingValueAfterJoin],
                ["turk", 2002, ErrorValues_1.ErrorValueTypes.NoMatchingValueAfterJoin, 77],
            ]);
        });
    });
});
...
//# sourceMappingURL=CoreTable.test.js.map