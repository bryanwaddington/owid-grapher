...
const ExplorerUrlMigrations_1 = require("./ExplorerUrlMigrations");
// ================================================
...
describe("legacyToGridCovidExplorer", () => {
    const migration = ExplorerUrlMigrations_1.explorerUrlMigrationsById[ExplorerUrlMigrations_1.ExplorerUrlMigrationId.legacyToGridCovidExplorer];
    const legacyUrl = Url_1.Url.fromURL("https://ourworldindata.org/coronavirus-data-explorer?country=ESP~MKD~North+America");
    ...
    it("sets new pathname", () => {
        expect(migratedUrl.pathname).toEqual("/explorers/coronavirus-data-explorer");
    });
    ...
    it("sets boolean options to 'false' if omitted in legacy URL", () => {
        const legacyUrl = Url_1.Url.fromURL("https://ourworldindata.org/coronavirus-data-explorer?casesMetric=true");
    ...
    it("sets default view for legacy URL without params", () => {
        const legacyUrl = Url_1.Url.fromURL("https://ourworldindata.org/coronavirus-data-explorer");
...
describe("co2 explorer", () => {
    const legacyUrl = Url_1.Url.fromURL("https://ourworldindata.org/explorers/co2?tab=chart&xScale=linear&yScale=linear&stackMode=absolute&time=earliest..latest&country=China~United%20States~India~United%20Kingdom~World&Gas%20=CO%E2%82%82&Accounting%20=Production-based&Fuel%20=Coal&Count%20=Cumulative&Relative%20to%20world%20total%20=Share%20of%20global%20emissions");
    const migratedUrl = ExplorerUrlMigrations_1.migrateExplorerUrl(legacyUrl);
...
describe("energy explorer", () => {
    const legacyUrl = Url_1.Url.fromURL("https://ourworldindata.org/explorers/energy?tab=chart&xScale=linear&yScale=linear&time=earliest..latest&country=United%20States~United%20Kingdom~China~World~India~Brazil~South%20Africa&Total%20or%20Breakdown%20=Select%20a%20source&Select%20a%20source%20=Fossil%20fuels&Energy%20or%20Electricity%20=Electricity%20only&Metric%20=Per%20capita%20generation");
    const migratedUrl = ExplorerUrlMigrations_1.migrateExplorerUrl(legacyUrl);
    it("generates correct query params", () => {
        expect(migratedUrl.queryParams).toEqual({
            ...
            country: "United States~United Kingdom~China~World~India~Brazil~South Africa",
            tab: "chart",
            time: "earliest..latest",
            xScale: "linear",
            yScale: "linear",
...
describe("covid explorer", () => {
    it("migrates Vaccinations", () => {
        const legacyUrl = Url_1.Url.fromURL("https://ourworldindata.org/explorers/coronavirus-data-explorer?Metric=Vaccinations");
        const migratedUrl = ExplorerUrlMigrations_1.migrateExplorerUrl(legacyUrl);
    ...
    it("migrates Tests per confirmed case", () => {
        const legacyUrl = Url_1.Url.fromURL("https://ourworldindata.org/explorers/coronavirus-data-explorer?Metric=Tests+per+confirmed+case");
        const migratedUrl = ExplorerUrlMigrations_1.migrateExplorerUrl(legacyUrl);
        expect(migratedUrl.queryParams).toMatchObject({
...
//# sourceMappingURL=ExplorerUrlMigrations.test.js.map