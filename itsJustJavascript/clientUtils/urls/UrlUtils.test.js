...
describe(UrlUtils_1.queryParamsToStr, () => {
    for (const testCase of testCases) {
        if (testCase.ignoreInToQueryStrTest)
            continue;
        it(`can convert query params to a query string: '${testCase.queryStr}'`, () => {
            expect(UrlUtils_1.queryParamsToStr(testCase.queryParams)).toEqual(testCase.queryStr);
        });
    }
});
describe(UrlUtils_1.strToQueryParams, () => {
    for (const testCase of testCases) {
        it(`can convert query string to a query params object: '${testCase.queryStr}'`, () => {
            expect(UrlUtils_1.strToQueryParams(testCase.queryStr)).toEqual(testCase.queryParams);
            expect(UrlUtils_1.strToQueryParams(testCase.queryStr, true)).toEqual(testCase.encodedQueryParams);
        });
    }
});
