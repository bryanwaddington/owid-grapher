...
function importCodebook() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const now = new Date();
        const codebookRows = yield csv_1.parseCSV(codebookCSV);
        ...
        const variablesByCode = lodash.keyBy(vdemVariables.filter((v) => v.shortDefinition), (v) => v.indicatorCode);
        ...
        const userId = (yield db.mysqlFirst(`SELECT * FROM users WHERE fullName=?`, [
            "Jaiden Mispy",
        ])).id;
        yield db.transaction((t) => __awaiter(this, void 0, void 0, function* () {
            const existingDataset = (yield t.query("SELECT id FROM datasets WHERE namespace='vdem'"))[0];
            if (existingDataset) {
                yield t.execute(`DELETE d FROM data_values AS d JOIN variables AS v ON d.variableId=v.id WHERE v.datasetId=?`, [existingDataset.id]);
                ...
                yield t.execute(`DELETE FROM datasets WHERE id=?`, [
                    existingDataset.id,
                ]);
            }
            const datasetRow = [
            ...
            const result = yield t.query("INSERT INTO datasets (namespace, name, description, isPrivate, createdAt, updatedAt, metadataEditedAt, metadataEditedByUserId, dataEditedAt, dataEditedByUserId, createdByUserId) VALUES (?)", [datasetRow]);
            ...
            for (let i = 0; i < vdemVariables.length; i++) {
                ...
                const sourceDescription = {
                    ...
                    link: string_1.findUrlsInText(v.variableSource).join(","),
                ...
                const sourceResult = yield t.query("INSERT INTO sources (datasetId, name, createdAt, updatedAt, description) VALUES (?)", [sourceRow]);
                ...
                yield t.query("INSERT INTO variables (datasetId, sourceId, columnOrder, name, code, description, createdAt, updatedAt, originalMetadata, unit, coverage, timespan, display) VALUES (?)", [variableRow]);
            }
        }));
    });
}
function importData() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield db.transaction((t) => __awaiter(this, void 0, void 0, function* () {
            yield t.execute(`DELETE dv FROM data_values dv JOIN variables v ON dv.variableId=v.id JOIN datasets d ON v.datasetId=d.id WHERE d.namespace='vdem'`);
            const now = new Date();
            ...
            const variableRows = yield t.query(`SELECT v.code, v.id FROM variables v JOIN datasets d ON d.id=v.datasetId WHERE d.namespace='vdem'`);
            ...
            const csv = new csv_1.CSVStreamParser(input);
            let row = yield csv.nextRow();
            while (row !== undefined) {
                if (!variableIds.length) {
                    ...
                    const year = row[3];
                    for (let index = VARIABLE_START_COLUMN; index < row.length; index++) {
                        const variableId = variableIds[index];
                        if (!variableId || row[index] === "")
                            continue;
                        valueRows.push([
                            row[index],
                            parseInt(year),
                ...
                row = yield csv.nextRow();
            }
            // Final values
            if (valueRows.length)
                yield insertRows();
            // Delete variables from the codebook with no data
            yield t.execute("delete v from variables v left join data_values dv on dv.variableId=v.id join datasets d on d.id=v.datasetId where d.namespace='vdem' and dv.id IS NULL;");
        }));
    });
}
function main() {
...
main();
//# sourceMappingURL=importVDemDataset.js.map